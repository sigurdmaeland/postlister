...
Tilsynssak) skrapes fra samme portal med identisk logikk - eneste reelle
forskjell er saksnummer-prefikset ("BYGG-"/"HENV-"/"ULOV-"/"TILSYN-") brukt
i søket, gitt som parameter til hver funksjon.
"""
...
# --------------------------------------------------------------------------- #
# Konstanter felles for alle sakstyper i Lillestrøm
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Lillestrøms rå adressefelt er en liste med linjer som "Gatenavn 12, 4610
# LILLESTRØM, Norge" - ofte med duplikater (samme adresse gjentatt med og
# uten mellomrom i bokstav-suffikset, "16 B" vs "16B"), og noen ganger med
# FLERE husnumre kommaseparert på én linje. Funksjonene under rydder dette
# opp til en flat, deduplisert adresseliste, slik at "adresse"/"gnr_bnr"
# lagres som semikolon-separerte strenger og et "matrikkelnr"-felt kan
# avledes.
...
def parse_title_matrikkel(tittel):
    ...
    oppgir aldri feste/seksjon (kun gnr/bnr) - tittelen
    ("GNR/BNR/FESTENR/SEKSJONSNR Gatenavn Nummer, beskrivelse") er eneste
    kilde til den presisjonen. "0/0" er placeholder for "ingen
    feste/seksjon" og hoppes over - kun reelle verdier tas med."""
    result = {}
    if not tittel:
        return result
...
def build_matrikkelnr(gnr_bnr_list, kommune_nr, tittel=None):
    """Bygger matrikkelnr fra deduplisert gnr_bnr-liste, f.eks. "3205-150/732" -
...
# stående tegn (bekreftet på Kristiansands identiske plattform/kode - se
# rapport). Kun ordet FORAN selve husnummeret sjekkes (bokstav-suffiks på
# husnummeret selv, f.eks. "16B", er noe helt annet og skal IKKE avvises).
...
def _har_husnummer(addr):
    ...
    uten dette er kandidaten som regel et sone-/stedsnavn uten en
...
def address_from_tittel(tittel, kommune_nr):
    ...
    fallback når adresse-panelet på siden er tomt. Tre kjente formater: (1)
    "Gatenavn [Nummer] GNR/BNR[/FESTENR/SEKSJONSNR][,] beskrivelse"
    (vanligst), (2) "Gatenavn Nummer - GNR/BNR - melding om mulig ulovlig
    tiltak - ..." (eldre ulovlighetssaker; gnr/bnr kan mangle, vist som bar
    "/", eller vises som kommunenr/gnr/bnr), (3) "GNR/BNR/FESTENR/SEKSJONSNR
    Gatenavn Nummer, beskrivelse" - matrikkelnummeret FØRST i tittelen i
    stedet for etter gateadressen (bekreftet forekommende format - uten
    denne grenen tolkes hele biten før komma som en tom adresse siden
    matrikkelnummeret selv står helt i starten av tittelen). Hvis tittelen
    ikke har noe gnr/bnr i det hele tatt, sjekkes om starten av tittelen
    likevel ser ut som en ren adresse ("Gatenavn Nummer, beskrivelse", typisk
    for henvendelser/bevillinger uten matrikkeltilknytning). Kandidaten
    kjøres gjennom samme flerdresse-splitting som adresse-panelet
    (_parse_address_parts), slik at "Gate 4 og 12" blir to adresser i stedet
    for én sammenslått streng. Returnerer None for adresse hvis kandidaten
    ikke har et ekte husnummer til slutt."""
    if not tittel:
        return None, None
    ...
    # det for HELE tittelen (f.eks. "75/329/0/0 Ingen adresse - Tomt
    # Breidablikkveien 2, ...") - i så fall forkastes den helt i stedet for
    # å prøve å plukke ut en "ekte" adresse etter frasen, i tråd med samme
...
def extra_gnr_bnr_fra_tittel(tittel):
    ...
    426/47, 426/71, 426/54 (bekreftet på Kristiansands identiske
    plattform/kode - se rapport). Disse sakene gjelder flere eiendommer
    samtidig - det andre (og evt. tredje osv.) paret finnes ALLTID i
    TILLEGG til det adresse-panelet allerede måtte ha gitt (som typisk kun
    viser det første), derfor kalles denne alltid fra parse_case, uavhengig
    av om adresse/gnr_bnr allerede er fylt ut fra andre kilder. Stopper ved
    første ledd som ikke selv er et nytt matrikkelnummer (fullt par ELLER
    bart bruksnummer), for å unngå å plukke opp uskyldige tall lenger ute i
    beskrivelsesteksten."""
    if not tittel:
        return []
...
# har aldri dette rett før et tall (bekreftet på Kristiansands identiske
# plattform/kode - se rapport).
...
# skriver kommunenummeret FØRST i kjeden (f.eks. "3205/13/143" =
# KOMMUNE_NR/gnr/bnr, ikke gnr/bnr 3205/13 - samme mønster som Trondheim/
# Tromsø/Drammen/Kristiansand).
...
def _gnr_bnr_fra_tallkjede(tallkjede):
    ...
    "3205/472" - kun kommunenr+gnr, ingen bnr - en ufullstendig/redundant
...
def get_case_list(saksnummer_prefiks):
    ...
    sistnevnte viser stille kun "siste to måneder" som standardfilter (samme
    kjente feil som i Kristiansand). "q="-søket viser "Alle" år/sakstype som
    standard (bekreftet: BYGG=8601, HENV=2204, ULOV=545, TILSYN=192 treff),
    ingen paginering."""
...
def parse_case(html, document_id):
    ...
    if not adresse_clean and fallback_adresse:
        ...
        # for en annen gate (bekreftet på Kristiansands identiske
        # plattform/kode - se rapport).
...
# --------------------------------------------------------------------------- #
# Små JSON-IO-hjelpere, delt av begge kjøremåtene under
# --------------------------------------------------------------------------- #
def _read_json_or_default(path, default):
    """Leser og parser en JSON-fil, og returnerer `default` hvis fila ikke
    finnes eller ikke kan parses (f.eks. avbrutt/korrupt fra en tidligere
    kjøring)."""
    if not path.exists():
        return default
    try:
...
def _atomic_write_json(data, path, **dump_kwargs):
    """Skriver `data` som JSON til `path` via en midlertidig fil, for å unngå
    halvskrevet output ved avbrudd underveis."""
...
def run_full_dump(output_file, saksnummer_prefiks, limit=None, save_every=200):
    ...
    måtte gå gjennom hele den tunge full-dumpen hver gang."""
