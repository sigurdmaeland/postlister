"""Delt bibliotek for Sarpsborg bygg-scraperen.

Samme OpenGov-plattform (360online) som Kristiansand/Lillestrøm - denne
filen er en tilpasning av lillestrom/common/scraper_lib.py med
Sarpsborg-spesifikke konstanter. Skrape-/ryddelogikk er ellers identisk.

Kun Byggesak (BYGG-) skrapes, per eksplisitt brukervalg (Kristiansand/
Lillestrøm skraper i tillegg henv/ulov/tilsyn).

FORSKJELL fra Lillestrøm: sakstittelens ordrekkefølge er snudd. Lillestrøm
skriver "Gatenavn Nummer GNR/BNR[/FESTENR/SEKSJONSNR], beskrivelse"
(gatenavn først), mens Sarpsborg skriver "GNR/BNR/FESTENR/SEKSJONSNR -
Gatenavn Nummer - beskrivelse" (matrikkelnr først, f.eks.
"2081/254/0/0 - Yvenveien 40 - tilbygg"). Se address_from_tittel() for
detaljer - denne er uansett bare en fallback som sjelden trigges, siden
adresse-panelet på detaljsiden nesten alltid gir adresse+gnr/bnr direkte.

Brukes av:
  - 2023-today_dump/bygg/main.py   (engangs historisk dump)
...
SEARCH_URL = f"{BASE_URL}/Cases/sarpsborg"
CASE_BASE_URL = f"{BASE_URL}/Cases/sarpsborg/Case/Details/"

KOMMUNE_NR = 3105   # Sarpsborg (endret fra 3003 ved kommunenummer-endringen i 2024)
KOMMUNE = "Sarpsborg"
...
# --------------------------------------------------------------------------- #
# Sarpsborgs rå adressefelt er en liste med linjer som
# "Gatenavn 12, 1706 SARPSBORG, Norge" - noen ganger duplikater (samme
...
# mønster som ble fikset for Lillestrøm/Kristiansand/Trondheim. Funksjonene
# under rydder dette opp til en flat, deduplisert adresseliste, slik at
# "adresse"/"gnr_bnr" lagres som semikolon-separerte strenger, og et
...
def _parse_address_parts(s):
    ...
    og/eller ' og '. Et bart tall/spenn arver gatenavnet fra forrige element
...
# Sakstittelen skriver ALLTID fullt matrikkelnr FØRST ("GNR/BNR/FESTENR/
# SEKSJONSNR - Gatenavn ..."), stikk i strid med Lillestrøm der det kommer
# etter gatenavnet. "0/0" er kildesystemets plassholder for "ingen feste/
# seksjon" (brukt i det store flertallet av titler) og skal IKKE skrives ut
# i matrikkelnr, kun de sjeldnere tilfellene med en reell verdi.
_MATRIKKEL_TITLE_RE = re.compile(r"^(\d+)/(\d+)(?:/(\d+)(?:/(\d+))?)?\s*-")


def parse_title_matrikkel(tittel):
    """Finn gnr/bnr -> (festenr, seksjonsnr) fra STARTEN av sakstittelen, når
    minst ett av dem faktisk er != "0" (se modul-docstringen for
    bakgrunn)."""
    result = {}
    if not tittel:
        return result
    m = _MATRIKKEL_TITLE_RE.match(tittel.strip())
    if not m:
        return result
...
def build_matrikkelnr(gnr_bnr_list, kommune_nr, tittel=None):
    """Bygger matrikkelnr fra deduplisert gnr_bnr-liste, f.eks. "3105-2081/254"
    - berikes med feste/seksjon parset fra sakstittelen når det finnes og
    faktisk er != 0/0, f.eks. "2063/1/315/0 - Prost Krogs vei 15 - ..." ->
    matrikkelnr "3105-2063/1/315/0" i stedet for kun "3105-2063/1". gnr_bnr-
...
# "GNR/BNR/FESTENR/SEKSJONSNR - Gatenavn [Nummer] - beskrivelse" - det
# klart vanligste formatet for Sarpsborgs byggesaker (stikkprøve av 30
# tilfeldige ekte titler fulgte dette mønsteret konsekvent).
_GNR_BNR_TITLE = re.compile(r"^(\d+)/(\d+)(?:/\d+){0,2}\s*-\s*")
# Et lite mindretall titler (typisk infrastruktur-/VA-anlegg-saker uten
# noen enkelt gateadresse) bryter dette mønsteret og har gnr/bnr et annet
# sted i tittelen i stedet - se _GNR_BNR_ANYWHERE-bruken under.
_GNR_BNR_ANYWHERE = re.compile(r"\b(\d{1,4})/(\d{1,5})\b")
...
def _har_husnummer(addr):
    ...
    selvstendig identifiserbar adresse (f.eks. "Skjebergveien" alene)."""
    return bool(_HAR_HUSNUMMER.search(" " + addr))


def address_from_tittel(tittel):
    """Utleder (adresse, gnr_bnr_liste) direkte fra sakstittelen - brukes KUN
    som fallback når adresse-panelet på siden ikke ga noe. Vanligste format
    (matrikkelnr FØRST, ulikt Lillestrøm/Kristiansand - se modul-docstring):
    "GNR/BNR/FESTENR/SEKSJONSNR - Gatenavn Nummer - beskrivelse", f.eks.
    "2081/254/0/0 - Yvenveien 40 - tilbygg". Tar kun med FØRSTE gate-segment
    ved flere adresser i samme tittel.

    Et lite mindretall (~2%, typisk VA-/infrastruktursaker uten én enkelt
    gateadresse) følger ikke dette mønsteret - da søkes hele tittelen etter
    alle "tall/tall"-par i stedet (trygt her siden saksnummeret alltid har
    et bokstavprefiks og aldri kan forveksles med et bart gnr/bnr-par).

    Returnerer None for adresse hvis kandidaten ikke har et ekte husnummer
    til slutt, eller er "Ingen adresse"."""
    if not tittel:
        return None, None

    gnr_bnr_liste = []
    rest = tittel.strip()
    m = _GNR_BNR_TITLE.match(rest)
    if m:
        gnr_bnr_liste = [f"{m.group(1)}/{m.group(2)}"]
        rest = rest[m.end():]
    else:
        for gnr, bnr in _GNR_BNR_ANYWHERE.findall(tittel):
            par = f"{gnr}/{bnr}"
    ...
    if forste_segment and not _INGEN_ADRESSE.match(forste_segment):
        normalized = _normalize_nummer_bokstav(forste_segment)
        if _har_husnummer(normalized):
            adresse = normalized

    return adresse, (gnr_bnr_liste or None)


# Saksnummer-prefikset koder sakstypen, f.eks. "BYGG-26/00840" -> Byggesak.
# Kun BYGG- skrapes her, men beholder oppslaget for evt. fremtidig utvidelse
# og for den defensive sjekken i _forkast_uventet_sakstype under.
...
# --------------------------------------------------------------------------- #
# Scraping / parsing - felles for begge kjøremåtene (engangs historisk dump
# og daglig endringslogg)
# --------------------------------------------------------------------------- #
def get_case_list(saksnummer_prefiks):
    ...
    som standard (bekreftet for Sarpsborg: BYGG=2651 treff, ingen paginering)."""
...
def parse_case(html, document_id):
    ...
    # siden ikke ga noe - fyller kun inn hull, overstyrer aldri det panelet
    # faktisk ga.
    if not adresse_clean or not gnr_bnr_clean:
        fallback_adresse, fallback_gnr_bnr_liste = address_from_tittel(sakstittel)
        if not adresse_clean and fallback_adresse:
            adresse_clean = [fallback_adresse]
        if not gnr_bnr_clean and fallback_gnr_bnr_liste:
            gnr_bnr_clean = fallback_gnr_bnr_liste
...
# --------------------------------------------------------------------------- #
# Engangs historisk dump (2023-today_dump) - gjenopptakbar kjøring
# --------------------------------------------------------------------------- #
def _load_json(path, default):
    """Leser og parser en JSON-fil trygt - returnerer default hvis fila ikke
    finnes eller innholdet ikke kan parses."""
    if not path.exists():
        return default
    try:
...
def _atomic_write_json(data, path, **dump_kwargs):
    """Skriver JSON til disk via en midlertidig fil for å unngå halvskrevet output."""
...
def run_full_dump(output_file, saksnummer_prefiks, limit=None, save_every=200):
    """Full historisk dump av bygg-sakene (brukes av 2023-today_dump/bygg/main.py).

    limit=N henter kun de N nyeste (lista er sortert nyeste først)."""
...
def full_sweep(saksnummer_prefiks, limit=None):
    """Hent alle sakene av bygg-sakene på nytt (full skanning).
...
def load_snapshot(snapshot_file):
    ...
    return _load_json(snapshot_file, default={})
...
def upload_til_azure(saker_file, jp_file, ref_date, sakstype):
    """Opplastings-hook mot Azure Blob - IKKE IMPLEMENTERT ENNÅ. Sentralisert
    her slik at det er ETT sted å koble på ekte opplasting når Azure-
    oppsettet er klart. Planlagt stikonvensjon:
    bronze/{kommune}/{sakstype}/load_type=incremental/date=<ref_date>/."""
...
def run_daily(snapshot_file, output_dir, saksnummer_prefiks, sakstype, limit=None, today_fn=i_dag_oslo):
    """Daglig endringslogg for bygg-sakene (brukes av running_daily/bygg/app/main.py).

    `today_fn` er injiserbar for testing (simulere flere "dager" uten å
    vente på ekte klokketid)."""
