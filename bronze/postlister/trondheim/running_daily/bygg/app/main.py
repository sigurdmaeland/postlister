...
# Daglig endringslogg for Trondheim-byggesaker (GraphQL, 360 Plan & Build).
#
# Ekte datoer, så dette er dato-basert (som Bærum) – ingen snapshot trengs. Se
# fulldump-scriptet (2017-03-24-today_dump/bygg/main.py) for detaljerte notater
# om API-begrensningene (offset-paginering er ødelagt server-side, maks
# limit=100/felt, query-teksten må sendes byte-eksakt). Datovindu = i går
# (WINDOW_DAYS=1). De fleste dager er godt under 100 saker/journalposter; et
# fåtall over grensen fylles etter med departement-filter.
...
def write_output(nye_saker, nye_journalposter, ref_date):
    ...
    # TODO: last opp som .jsonl.gz til Azure (bronze/trondheim/load_type=incremental/date=<ref>/)
...
def run(day_back=1):
    ...
    #    sjelden truffet (motsatt av Bærum), men beholdes som fallback.
    nye_journalposter = []
