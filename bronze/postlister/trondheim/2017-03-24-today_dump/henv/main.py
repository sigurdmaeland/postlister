...
# Full-dump av Trondheim-henvendelser fra Innsyn (GraphQL, samme plattform som
# ../bygg/main.py).
#
# I motsetning til Asker har Trondheim sin EGEN GraphQL-type for dette
# ("Henvendelse", type-id under) - ingen heuristikk trengs. Bekreftet ved
...
# og "Ingen registrering". Denne mappa dekker KUN Henvendelse-typen, på samme
# måte som ../bygg/main.py kun dekker Byggesak og ../ulov/main.py kun
# ulovlighetssak - egen mappe+fil per sakstype, som i Asker/Bergen.
#
# Sekvensnummer har prefiks "HENV-" i nyere saker (f.eks. HENV-26/82010), men
# eldre saker deler samme "BYGG-"-prefiks som byggesaker uansett type -
# prefikset alene kan altså ikke brukes til å skille sakstype, kun
# type.id/type.name-feltet fra API-et.
#
...
# saker fra andre datoer. De første nodene i lista treffer likevel korrekt for
# det spurte vinduet før den "lekker" utover - siden dag-for-dag-metoden
# (samme som ../bygg/main.py) går gjennom HVER dag i hele perioden, blir hver
# sak uansett fanget opp korrekt på sin egentlige dag, og eventuell "lekkasje"
# fra andre dager gir bare ufarlige duplikater (de dedupliseres på sak-id).
#
# Eldste henvendelse funnet i portalen: 2018-06-18 (BYGG-18/82521).

BASE_URL = "https://trondheim.innsynsportal.no"
API_URL = f"{BASE_URL}/graphql"
LIST_ID = "fc010204-2c07-4f3c-a8ef-879ec218111c"       # byggesak-postliste (delt med bygg/ulov)
HENV_TYPE_ID = "a7c85533-e802-4aa5-bee2-46a24fbed199"  # "Henvendelse"-type UUID
...
START_DATE = date(2018, 6, 18)   # eldste henvendelse funnet i portalen (BYGG-18/82521)
...
OUTPUT_FILE = HERE / "trondheim_henv.json"
...
def search_post_journal(session, *, plimit, jlimit, proceedings_where, journals_where,
    ...
    body = {
        ...
        "variables": {
            ...
            "journalProceedingWhere": {"typeIdIn": [HENV_TYPE_ID]},
...
def sak_url(saksnummer):
    params = json.dumps({"proceeding": {"sequenceNumber": saksnummer, "typeIdIn": [HENV_TYPE_ID]}})
...
if __name__ == "__main__":
    import sys
    # Valgfrie argumenter:
    #   python3 main.py                        -> full dump fra 2018-06-18
