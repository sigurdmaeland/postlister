...
def import_test_data():
    with open('../json_data/ULOV_bergen_postliste_2022-2025-11-02.json') as testfile:
...
def prepare_matrikkel(sak_uid, sak_kommunenavn, sak_kommunenr, sak_matrikkelnr, sak_gnrBnr):
        ...
        if sak_gnrBnr:
            ...
            for gnr_bnr_str in sak_gnrBnr:
                ...
                if len(parts) >= 2:
                    ...
                    bnr.append(None)  # Add None if missing
    ...
    for i in range(len(matrikkelnr)):
        ...
        matrikkel_data.append([
            ...
            sak_kommunenavn,
            int(sak_kommunenr),
            int(gnr[i]) if i < len(gnr) and gnr[i] else None,  # Added bounds check
            int(bnr[i]) if i < len(bnr) and bnr[i] else None,  # Added bounds check
...
def batch_copy(cur, saker_data, matrikkel_data, 
    ...
    copy_from_data(cur, 'postlister_dokument', 
                   ['uid', 'journalpost_uid', 'tittel', 'type', 'nummer'],
                   dokument_data)
...
def main():
    ...
    try:
        ...
        for batch_start in range(0, total_saker, BATCH_SIZE):
            ...
            batch_copy(cur, all_saker, all_matrikkel, 
