...
def fetch_bergen_cases(date) -> list:
    try:
        ...
        date_start = int((date + timedelta(days=-30)).timestamp() * 1000)  # 30 days back from the date we have set to check
...
async def main(day_back_from_today=1, ACCOUNT_URL='https://storaggen2eaccountprod.blob.core.windows.net', AZURE_CONTAINER_NAME='postlister', DATA_PATH='bronze/bergen/load_type=incremental'):
    print(f"[TZ DEBUG] datetime.now() (no tz):    {datetime.now()}")
    print(f"[TZ DEBUG] datetime.now(Oslo):         {datetime.now(ZoneInfo('Europe/Oslo'))}")
    print(f"[TZ DEBUG] date without tz - 1 day:    {(datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=day_back_from_today)).strftime('%Y-%m-%d')}")
    print(f"[TZ DEBUG] date with Oslo tz - 1 day:  {(datetime.now(ZoneInfo('Europe/Oslo')).replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=day_back_from_today)).strftime('%Y-%m-%d')}")
    ...
    print(f"[TZ DEBUG] bergen_reference_date:       {bergen_reference_date}")
    ...
    if ACCOUNT_URL:
        try:
            ...
            blob_name = f"{DATA_PATH}/date={bergen_reference_date.strftime("%Y-%m-%d")}/bygg_saker-{bergen_reference_date.strftime("%Y-%m-%d")}.jsonl.gz"
