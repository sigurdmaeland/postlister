...
# Daglig endringslogg for Asker-byggesaker (samme Innsyn-plattform som
# Trondheim/Drammen).
#
# Denne mappa dekker KUN BYGG_TYPE_ID ("eByggesak"). Ulovlighetssaker
# (Ulovlighetsoppfølging) hentes av det egne scriptet i ../../ulov/app/main.py,
# på samme måte som Trondheim/Bergen skiller bygg/ulov/henv i egne mapper
# med egne filer i easy-pipelines. Askers portal har ingen egne
# "Henvendelsessak"- eller "Tilsyn"-typer å filtrere på.
#
...
# saksnummer-match. Se 2020-today_dump/bygg/main.py for detaljerte notater.
#
# Som forsiktighetsregel (samme mønster som Drammen, ikke spesifikt bekreftet
# for Asker ennå) skanner hver kjøring et bredere tilbakeblikk (LOOKBACK_DAYS)
# og filtrerer mot seen_journalpost_ids.json, i tilfelle saksbehandlere ikke
# alltid registrerer alt samme dag. Output-filnavnet er fortsatt datert på
# referansedagen (ref), så en evt. Azure-partisjonering
# (bronze/asker/load_type=incremental/date=<ref>/) er upåvirket.
...
# "Mulig ulovlighet", "Forespørsel om ..."). extract_adresse() rydder bort
# alt dette og returnerer None i stedet for å gjette når ingenting reelt
# gjenstår. Kilden bruker begge separator-varianter (vanlig bindestrek "-"
# og tankestrek "–").
...
def write_output(saker, ref_date):
    ...
    # TODO: last opp som .jsonl.gz til Azure (bronze/asker/load_type=incremental/date=<ref>/)
